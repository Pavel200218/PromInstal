...
function t_handleGoogleApiReady(recid) {
    ...
    Array.prototype.forEach.call(gmaps, function (gmap) {
        ...
        var mapZoomAttr = gmap.getAttribute("data-map-zoom"),
            ...
            myOptions = { zoom: parseInt(mapZoomAttr, 10), center: myLatLng, scrollwheel: !1, gestureHandling: "cooperative", zoomControl: !0, styles: mapstyle },
        ...
        if (
            (Array.prototype.forEach.call(arMarkers, function (el) {
                (myLatLng = new google.maps.LatLng(parseFloat(el.lat), parseFloat(el.lng))),
                    (marker = new google.maps.Marker({ position: myLatLng, map: map, title: el.title })),
                    bounds.extend(myLatLng),
                    el.descr ? t_map__attachInfoMessage(marker, el.descr) : t_map__attachInfoMessage(marker, el.title);
            }),
            arMarkers.length > 1)
        ) {
...
function t_handleYandexApiReady(recid) {
    ...
    Array.prototype.forEach.call(ymapsArr, function (ymap) {
        ...
        var myLatLng = arMarkers.length > 0 && [parseFloat(arMarkers[0].lat), parseFloat(arMarkers[0].lng)],
            myOptions = { zoom: parseInt(mapZoomAttr, 10), center: myLatLng, scrollZoom: !1, controls: ["typeSelector", "zoomControl"], drag: window.isDragMap },
        ...
        Array.prototype.forEach.call(Object.keys(mobilePanelStyles), function (name) {
            ...
            Array.prototype.forEach.call(arMarkers, function (marker) {
                ...
                (textarea.innerHTML = marker.descr), myGroup.add(new ymaps.Placemark(myLatlng, { hintContent: marker.title, balloonContent: marker.descr ? textarea.textContent : marker.title }));
            }),
            map.geoObjects.add(myGroup);
        var zoom = parseInt(mapZoomAttr, 10);
        arMarkers.length > 1
            ? (map.setBounds(myGroup.getBounds(), { checkZoomRange: !0 }).then(function () {
